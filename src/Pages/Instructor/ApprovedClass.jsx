...
const ApprovedClass = () => {
  ...
  useEffect(()=>{
    axiosSecure.get(`/approvedclass/${currentUser.email}`).then((res)=>{
      console.log(res)
      setClasses(res.data)
      setloading(false)
  ...
  return (
    <div className='my-10'>
      <div className='text-center'>
        <h1 className='text-4xl font-bold'>Approved  <span className='text-secondary'>classes</span></h1>
        <p className="text-[12px] text-center my-2">
        Here you can see the classes that are approved by admin.
      </p>
      {classes.length === 0 ? (
        <div className="text-center text-2xl font-bold mt-10">
          You don't have any approved class yet....
        </div>
      ) : (
